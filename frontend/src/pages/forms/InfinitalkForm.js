...
function InfinitalkForm() {
  ...
  const [formData, setFormData] = React.useState({
    ...
    title: 'Infinitalk API-AI lip-sync generator',
    input: {
      resolution: '480p',
    }
  });
          ...
          <form className="modelForm" onSubmit={handleSubmit}>
            ...
            <File
              ...
              onChange={(ev) => {
                const file = ev.target.files[0];
                file.uri = URL.createObjectURL(file);
                handleChange('audio_url', file)
              }}
              onFileDelete={() => handleChange('audio_url', null)}
              accept="audio/*"
              text='MPEG, MP3'
            />
